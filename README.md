...
135:| **费率双模式判定函数 v1（本周 P0 落地）**（输入价差/价差中位/年化费率差/价差波动/持仓小时 → 输出套收敛/套费率/不进场 + 机器执行退出线=持仓×费率×1.2；5 场景全测：文档 5%+1bp/h 判套收敛双候选、横盘 15%/yr 判套费率退出线 14.79bps、妖币/费率不足/模糊判不进场） | `scripts/funding_mode_decision.py` |
