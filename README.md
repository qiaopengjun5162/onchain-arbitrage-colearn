...
142:| **PM 阶梯盘「卖 No」定价校准**（2025-04~08 五个月 BTC 阶梯盘 63 市场实测：0.90-0.995 No 带真实胜率 100% vs 隐含 92-98%=正 EV，**0.85-0.90 带真实 80% 负 EV=尾部雷区**（8月 $120K 真被触达）；合计偏差 +1.3%=薄利容量游戏；**卖 No 只做 0.90+**；坑=Gamma outcomePrices 是 JSON 字符串/CLOB history 用 token_id/防限流 sleep） | `scripts/pm_ladder_no_calibration.py` · `notes/pm-ladder-no-calibration-20260824.md` |
143:| **L0021 结营·知识图谱 v2**（官方线 21 天收官：五层图谱（概念/机制/工具/决策/执行）+ 存活地图 v2（清算事件✅/费率双模式✅/反向对冲❌/原子套利❌/PM卖No🟡/PM双买🟡）+ 判定支柱 v2（+收敛≠保证、+声称先核验）+ 8 cron 哨兵集群 + 双线结构；结营结论=个人 edge 收敛两条主线：清算事件+费率事件窗口） | `notes/l0021-knowledge-graph-v2-20260824.md` |
| **21 天总结报告（D21 结营）**（学到 7 条认知主干 / 验证 8 项 / 否定 8 项（跨所反向对冲/常驻价差/原子套利/PM卖No跨标的/三角/跨链/错误单/逆势加仓）/ 下一步 4 方向（清算事件最高优先）/ 全地图最终版 14 方向 了解度×兴趣度×可行性 0-5 打分；结营一句话=大众套利全死，活下来两条主线都是「事件窗口+前置情报」） | `notes/21-day-summary-20260825.md` |
| **Flashblocks 决胜层研究（清算线第 3 步）**（Base 2s 区块=10 个 200ms 子区块、先到先得>gas 竞价（010 实证：2.5 gwei 赚 $10,679 vs 25 gwei 晚一桶 $254）；进场路径四层设计（信号 T-30s 预言机节奏+HF 预计算 / 构建 T-1s onMorphoLiquidate 零本金 / 竞争 0-200ms 预提交 / 出场=出货深度天花板）；原型 `base_flashblocks_probe.py` 实测：pending 可见（18 滚动块号）、liquidate selector `0xe72c76fe`、**Chainlink 更新事件驱动非 30s 固定**（40 轮 0 命中）→ 信号链修正为高频偏离轮询；暂不实盘） | `notes/flashblocks-decisive-layer-20260825.md` |
| **PM 卖 No 跨周期验证（ETH 否决）**（ETH 2025-04~08 五个月阶梯盘 74 样本合计 -5.6% 负 EV vs BTC +1.3% → 0.90+ 卖 No 正 EV 是 BTC 特有，不能跨标的推广；卖 No 维持候补不实盘（仅 BTC 0.95+ 薄利）；2024 月度阶梯盘确认不存在） | `notes/pm-ladder-no-cross-validation-20260825.md` |
| **350ms slot 对 Jito 管线影响评估**（实测 epoch 1022 ≈390ms/slot 已生效；Block Engine 50ms 拍卖未变→管线无破坏，但延迟预算 -12.5%、公共 RPC 同块竞争出局，私有 RPC+WS+预构建是入场券；雷达 slot 阈值 10→8、BAM 35ms 保持观察） | `notes/solana/350ms-slot-jito-impact-20260825.md` |
| **主网迁移清单（D20 补做）**（判定：只读监控类（雷达/费率/执行质量/清算哨兵）全可迁移零资金风险现在就能做，执行类（bundle）暂缓；RPC/账户/Gas/告警/风控六铁律/$250 最小测试资金/五步迁移顺序） | `notes/solana/mainnet-migration-checklist-20260825.md` |
...
- `scripts/morpho_prey_radar_v2.py` + `notes/solana/prey-radar-v2-highfreq-20260826.md` — prey radar v2 高频偏离轮询原型（结营后清算事件线第一步）：watchlist 12 市场批量 eth_call + 批量 DeFiLlama，有效节奏 2-3s/tick（v1 全市场串行 >180s 超时的 60 倍+）；`--once` 单次快照 / `--duration N --quiet` 窗口报警；实测 45s 降采样 22 条（vs 全量 540），安静期漂移 ±1.7bps 零误报；⚠️ DeFiLlama 批量 >10 币空、checksum 大小写 cache miss 两坑已修；限制=公共 RPC 节奏上限、未接 HF scanner（EUR 报价预言机已修，12/12 全解析）
...
- `notes/chainfeeds-daily-20260828.md` — ChainFeeds 早报 digest：glassnode BTC 清算燃料地图（8.1-8.6万阻力区/6.05-6.24万多头清算区/费率转负=反开确认）+ 社交交易 DAU 6-10万（听风生态支撑）+ CZ/Agent×Crypto/Zama 认知类
- `notes/ipo-convergence-arb-h1-20260828.md` — H1 上市收敛套利三案例（海力士跨市场/SPCX 6-12 IPO 收敛/Cerebras 5-14 IPO 收敛）：合成股份 IPO 收敛剧本（做空合成+买正股，100-250bps 预期）；可执行=pre-IPO 上线公告+IPO 日历哨兵
- `notes/boros-crossex-funding-arb-20260828.md` — Pendle Boros×CrossEx 核验：费率掉期四腿机制（HL空/Gate多/Boros YU双向，3.2% spread→13.9x 杠杆=27% APR）；判定=机制真实但杠杆违背蚂蚁仓铁律、红包是引流、辖区待确认
...
- `notes/yourquantguy-entropy-arb-code-20260828.md` — yourQuantGuy entropy-arb 代码发布核验（预热帖落地，MIT/80★/源码审读：风控骨架 5 件套可抄=persistence arming/库存阶梯/订单预算/故障暂停/链上对账 + record-only→analyzer 参数闭环；判定学习价值高不装不跑，执行层设计模板）
- `notes/group-share-ema-chart-reading-20260828.md` — EMA 读图教学帖（48Kchunbai/WiseInvest513 转发）：强/弱决策信号分层=信息差速度分层同构
- `notes/group-share-rl-agent-skfolio-20260828.md` — RL 交易 Agent 教程+skfolio（zheergen）：样本外验证防过拟合=回测铁律互证，skfolio 记工具备选
- `notes/group-share-buy-when-ignored-20260828.md` — 买在无人问津（WiseInvest513 #2）：存储周期股 MU/SNDK/SKHYNIX 入币股 RWA 候选池
