...
139:| **跨所永续价差收敛率回测（Bruce 收敛假设实证）**（Bybit×OKX 14 天 4032×2 根：SOL/DOGE 价差 ±8bps 内零事件=被磨平；KAITO max 310.8bps 但 ≥30bps 事件 4h 严格收敛率 0%、收窄一半也仅 25-43%；**事件窗口版追加：12 个 5m 内 ≥50bps 瞬态突变 2h 严格收敛 0%、收窄一半 33%——事件驱动假设在 CEX 永续也被证伪**（DEX 有锚点拉回、CEX 两所各自价格发现无锚点）；执行含义=反向对冲只做事件驱动或改套费率模式） | `scripts/cross_exchange_convergence_test.py` · `notes/cross-exchange-convergence-test-20260824.md` |
