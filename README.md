...
139:| **跨所永续价差收敛率回测（Bruce 收敛假设实证）**（Bybit×OKX 14 天 4032×2 根：SOL/DOGE 价差 ±8bps 内零事件=被磨平；KAITO max 310.8bps 但 ≥30bps 事件 4h 严格收敛率 0%、收窄一半也仅 25-43% → 常驻价差不收敛=结构性价差，收敛只在事件窗口；与 D17 同构；执行含义=反向对冲只做事件驱动或改套费率模式） | `scripts/cross_exchange_convergence_test.py` · `notes/cross-exchange-convergence-test-20260824.md` |
