...
| **Term Finance 治理攻击链上取证（$8.5M）**（低价收治理代币→恶意提案→金库 Safe 启用恶意模块→绕过签名提款；RPC 取证：Tx1 EnabledModule(0x0ae12af 自研模块) + Tx2 7 个 Safe 各启 1 模块提取 USDC ~$2.55M 流向攻击者；与 BonkDAO $20M/Compound $24M/BarnBridge 同族（8 周 7 协议 $22M）；=规则套利恶意版+事件驱动素材，待办=事件雷达加治理攻击类别） | `notes/term-finance-governance-attack-forensics-20260823.md` |
