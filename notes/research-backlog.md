...
- **资金费套利 = 卖保险**：收确定的保费（资费），扛不确定的赔付（暴涨）；赔付上限（保证金）才是生死线，保费只是零花钱。
- **正资金费率可以是诱饵，不是安全信号**：TUT 剧本（拉盘期 BG/Gate 日 8% 资费诱套利者做空→插针爆做空腿→剩单腿→补刀）；BTW 观察（+68.86% 暴拉/资费年化 124%/KOL 推广）高度相似。用 CrossEx 跨所保证金也防不了插针（2 倍杠杆照样爆）。
- **入场窗口**：冲高回落横盘后吃资费最稳（波动率降→方向性风险≈0，多头仓位没走→资费维持高位）；「资费高≠机会好，波动降下来资费还高才是机会好」。

下一步（2026-08-20 已部分完成）：

- [x] 跨所套利风控框架 → `notes/crossex-funding-arb-risk-20260820.md`（CrossEx 参数表：蚂蚁仓/抗 2 倍涨幅≈无杠杆/1 倍涨幅强平/U 本位/买卖一价；数学验证：保证金 ≥ (1+MMR) × 名义；MMR 陷阱：强平价按维持保证金率算，山寨币 MMR 很高）
- [ ] 写一篇复盘：为什么模拟盘和实盘差距大。
- [ ] 列出跨所套利的 kill criteria。
- [ ] 对照 1Token 资金费套利文章，整理一份风控指标清单。
- [ ] CrossEx 账户开通 + API 接入，查目标合约实际 MMR（计划币种 BNC，妖币）
- [ ] 盲区：CrossEx API 用法/资金费率结算规则/跨所保证金共享的具体风控参数——需实测补齐
...
## 14. Meme 微观结构：5M 壳价 cohort + Dev 行为哨兵（2026-08-11 新增）

来源：@nine_DeFi 妖币埋伏观点（「市值越接近 5M 越难亏钱，只要不偷偷增发」）× @hunterweb303 Dev 出货状态机拆解 × 群复盘「个人活路在低容量/信息差」。

假设：

- 5M 附近市值的壳币（无增发）7/30 天存活率和收益分布优于高位币——「难亏钱」可以量化；
- 用 Dev 行为状态机（建仓→藏仓→试探→主卖→清仓）过滤后，存活率更高的子集对应「Dev 还在做盘」阶段；
- 增发/解锁是壳价崩塌的第一杀手，需要独立监控。

验证路径：

- cohort 分析：Helius 拉 ~5M 市值币集合（历史快照），按 [mint 年龄、持有人数、增发事件、Dev 动作状态] 分层，统计 7/30 天存活率 + 收益分布；
- 增发监控：mint authority 是否可改 + 实际增发事件（getTokenSupply 变化）——「偷偷增发」的直接检测；
- Dev 状态机：把四本账（库存/距前高/净流/受控动作间隔）做成「Dev 行为哨兵」v1，输入 Dev 地址输出状态 + 告警；
- 输出：5M 壳价 cohort 存活率表 + Dev 状态标签，和 #13 低容量量化、backrun 薄池扫描共享基础设施。

风险提示：

- 反身性：埋伏策略拥挤后 Dev 会做假异动反收割（「你能想到的，庄家也早已料到」）；
- 这是博弈不是套利：无净收益公式、无可对冲腿，概率游戏——只做回测与哨兵，不做实盘建议；
- cohort 历史数据可得性：市值历史快照需要存档（Helius 无历史市值 API），先攒数据再分析。
