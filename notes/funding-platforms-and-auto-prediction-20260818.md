...
### 设计原理讲解（2026-08-18 作者长文，@0xcherry/status/2089588045922377738）

**常规 PM 套利三分类（作者归纳）**：
1. 单市场机械套利：YES+NO<1 就买、买到平衡为止 = 交易所搬砖，拼速度
2. 跨所同事件套利：同事件不同所定价差异做相反方向 = 拼速度
3. 预言机套利：预测 5 分钟后价格 vs 实际价格的建模/时间差

**核心论点**：以上传统方式**都没有状态**——不认为套利工程是可改进的状态机，因此无法用 AI 持续迭代（RSI），AI 在其中最多是「语义过滤器」。

**Auto Prediction 的本体论（有状态建模）**：
- 价格 = 预测市场概率的显性表达，价格即公允概率，并动态修正
- 事件的概率空间可展现为不同市场并构建概率关系（例：Trump 8 月遭枪击 vs 9 月访华 = 极强烈互斥事件）
- AI 的工作应持续改进建模 = 不断沉淀事件的状态表达

**工程**：Coding Agent 定期/长期派遣做市场事件搜索+联想，研究完成的事件沉淀下来；存在套利机会→提出套利方法；不存在→沉淀研究防止反复复工。

**声明**：纯研究项目、不保证收益；「事件-世界模型」可外放高价值研究，弥补传统交易难以纳入非量化数据的策略短板。姊妹项目 Auto Quant V2。
