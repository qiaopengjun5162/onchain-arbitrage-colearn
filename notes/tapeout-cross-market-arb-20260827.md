...
**流片费已核验（2026-08-28，官方白皮书 tapeout.net）**：
- 「流片」= 晶体管 Token 被**真实销毁**，电路 NFT 诞生——**无独立 BNB 流片费，仅 gas**；处理器创建费当前=0
- 组装成本 = directNand×NAND价 + directLatch×LATCH价（+gas 可忽略）→ `TAPEOUT_FEE_BNB` 已改为 0
- ⚠️ **关键新认知：各处理器的晶体管是不同 1155 合约**——Genesis CPU 晶体管合约 0x1d23Bf70（官方页明示），其 NAND 已铸满 1000 万、价格近零（0.000017 BNB）；TapeOut NAND 0.0077 BNB。**元件成本必须按处理器取价，跨处理器取价会系统性失真**（曾误用 Genesis CPU 近零价算 Behemoth 成本 → 虚报 28,000% 套利，修正后归零）
- 已知映射：Genesis CPU→0x1d23Bf70、TapeOut→0xCC42ba5D；Behemoth 晶体管合约未公开 → 回退 TapeOut 价（近似）
