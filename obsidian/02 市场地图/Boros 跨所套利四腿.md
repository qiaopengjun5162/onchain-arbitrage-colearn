...
tags:
  ...
  - funding-rate
  - pendle
source: notes/boros-pendle-crossex-4leg.md
---

## 一句话

Pendle CTO 开源的跨所套利 UI（CrossEx + Boros），把「Hyperliquid 空 ETH + OKX 多 ETH」的四腿策略做成一键工具，标注 **24.3% APR 固定收益**，到期日 9/25。
...
| 交易所腿 2 | 多 ETH | OKX | 杠杆 25x（可按风控调低） |
...
- 保姆级操作，每步都有市场链接直接点击
- 24.3% APR 是**宣传口径**，需 DYOR：费率会变、9/25 到期后续期条件未知

## 与知识库关联

- 同一工具家族：8/6 群分享的 yourQuantGuy 开源前端（gate-crossex）也是 CrossEx 生态——Pendle 这个是官方 CTO 出品的另一入口
- CrossEx 机制：跨所统一保证金，传统跨所波动 20% 单边爆仓，CrossEx 需价差拉大 20% 才爆仓
- 对照 [[跨所资金费率套利]] 的成本与风险模型
...
- 待办：实际装跑验证 CrossEx 体验——Boros 的 UI 比 gate-crossex 前端更「保姆级」，可优先试
