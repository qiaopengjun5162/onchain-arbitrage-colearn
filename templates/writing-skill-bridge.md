...
### 连接词级检查（2026-08-13 补充，来源 Roland.W《AI 写作问题的根源》）
AI 味的两个深层结构问题，比词汇替换更隐蔽，改稿时逐句过：

**① 自造说法**：需要读者先理解作者的说法、再理解事实本身的表达（「执行成本塌方」「先蹲两周侦察」），一律删除或换回事实。判断标准：自造说法没有增加信息时，直接写事实。

**② 英译式话语连接**（主语错位 + 事实/解释/结论压一句）：
- **违禁连接词**：`反映出 / 体现出 / 表明了 / 揭示了 / 这意味着 / 由此可见 / 说明了`——删掉后分别读前后两句，前句是可确认事实吗？后句是事实还是解释？原材料支持这层关系吗？不支持就删解释。
- **修复规则**：原材料确认原因 → 写「原因是」；仅同期出现 → 写「与……同时出现」；材料只支持一种可能 → 写「可能与……有关」；两项事实并列 → 拆成两句。
- **合规自查 regex**：`反映出|体现出|表明了|揭示了|这意味着|由此可见|说明了`，正文 0 处才算过。

**③ 协作姿势**：不让 AI 直接「润色」或「改得更像人写的」（左手倒右手改不干净）——让它把「原句 / 问题 / 建议表达」并排输出，人来核对它删了什么、有没有补新的判断。生成前可让 AI 先把原材料分三类：已确认事实 / 可由事实支持的解释 / 材料未回答的问题，分类后哪些直接写、哪些必须标明是判断，一目了然。
