# 资料链接

## 共学

- 链上套利残酷共学: https://intensivecolearn.ing/programs/b43d2e97-ed88-4ca3-b12f-7ef672b01205
- **taoli.tools 九种套利角度筛选结论**（作者社区角度闭合检查器五问筛选：4 当场砍/4 实测证伪/1 拥挤；单所绝对值 vs 跨所差 68 倍差距→SNDK 0.7% vs 47.7%；逃票判据=资金费>2×往返费；BNB IDO 168 天零异常证伪方法「绕开不知道的量」；cg-screen 22所×1848币一条命令）：本仓库 `notes/taoli-nine-arb-angles-filter-digest-20260815.md` / `sources/九种套利角度-筛选结论_20260815.pdf`
- **三角套利 TG 机器人骗局受害者陈述**（@RAY10168 8-13：套利豪仔 @pritipatelfgoo 推 TG 机器人→诱导输入私钥→4.1 BNB 秒空；「手续费 1%」实扣本金 1%；⚠️ 该骗子即 8-07 Polymarket 数学笔记分享者=人设立信闭环）：https://x.com/ray10168/status/2087914974215827778 → 本仓库 `notes/tg-scam-victim-statement-and-icefire-tools-20260815.md` / `sources/tg-triangle-arb-scam-ray10168_20260815.json`（关联 `notes/haasonline-scam-bot-case-20260814.md`）
- **冰火岛社区 GitHub 开源套利/期权工具清单**（@wepoets1107 张无忌 30 仓库：options-eye 11★/backpack-arbitrage 44★/btc-yield-enhancer 68★ 等；5 个代表性仓库已验真：真实存在+代码可审计+默认 dry-run+key 走环境变量）：https://x.com/wepoets1107/status/2088217033557315961 → 本仓库 `notes/tg-scam-victim-statement-and-icefire-tools-20260815.md`
- Bruce 使命宣言《我想对抗数字世界的失控》（LXDAO/ETHPanda/LI.FI/Hermes 的共同主线）：https://x.com/brucexu_eth/status/2049767573278068872 → 本仓库 `notes/bruce-mission-manifesto.md`
- Bruce 翻译 Greenfield《以太坊区块构建现状 Part I：需求侧——订单流与优先费》（MEV 三市场/优先费/OFA）：https://x.com/brucexu_eth/status/2085547569577627715 → 本仓库 `notes/ethereum-block-building-part1-demand.md`
- Bruce 训练 Hermes 写作 Skill 方法论（50 条样本→benchmark→Context 注入，70% 可用）：https://x.com/brucexu_eth/status/2083919618729820229
- Greenfield 以太坊区块构建 Dune 仪表盘（持续维护，全部图表数据源）：https://dune.com/gcm/ethereum-block-building
- CoW DAO Research《Private MEV Protection RPCs: Benchmark Study》（OFA 返现机制原始研究，80% 私有 RPC）：https://arxiv.org/abs/2505.19708 → 本仓库 `notes/arxiv-private-mev-protection-rpcs.md`
- Polymarket NegRisk Adapter「NO 回收套利」（0x 哆啦A梦分析 + arXiv 2608.00666，108.6 万 USDC 机制套利利润）：https://x.com/hunterweb303/status/2085552085618340330 → 本仓库 `notes/polymarket-negrisk-no-recycle-arbitrage.md`
- Polymarket 套利数学原理（Marginal Polytope/Bregman Projection/Frank-Wolfe，同论文数学向，4000 万美元）：https://x.com/pritipatelfgoo/status/2085560709115678803 → 本仓库 `notes/polymarket-arbitrage-math-framework.md` ⚠️ **2026-08-15 标记：分享者 @pritipatelfgoo（套利豪仔）已被证实为诈骗犯（TG 三角套利机器人骗私钥，受害者 4.1 BNB 被转空）——内容本身是 arXiv 论文搬运可独立核验，引用以论文为准**
- **Unravelling the Probabilistic Forest: Arbitrage in Prediction Markets**（AFT 2025/Flashbots FRP-51：Polymarket 一年 86.6M bids 实证，两类套利=Market Rebalancing（ΣYES≠$1）+Combinatorial（依赖市场对），已实现利润 ~$3,960 万、大头是「买 NO」$17.3M，约 99% 机会未被执行；LLM 找依赖市场对方法）：https://arxiv.org/pdf/2508.03474 → 本仓库 `notes/pm-probabilistic-forest-arb-paper-20260812.md` / `sources/papers/pm-probabilistic-forest-arb-2508.03474.pdf`
- Boros by Pendle 跨所套利四腿策略（CrossEx + Hyperliquid/OKX，24.3% APR，Pendle CTO 开源 UI）：https://x.com/pendle_grandma/status/2085369602360344631 → 本仓库 `notes/boros-pendle-crossex-4leg.md`
- 残酷共学官方 X @IntensiveCL: https://x.com/IntensiveCL
- Bruce 关于为什么发起共学的文章: https://x.com/brucexu_eth/status/2083935229757337948
- **Bruce 套利第一性原理长文**（官方教材：无套利一致性/资产闭环/全成本期望净利润；14 类套利分类+收益估算+速查地图）：https://x.com/brucexu_eth/status/2086652810905751610 → 待归档 `notes/bruce-first-principles-arbitrage-20260810.md`
- Bruce - Hermes Agent 配置指南（Setup/硬件/模型/Prompt）: https://x.com/brucexu_eth/status/2084604766714261729
- 蓝狐笔记 BIP-110 失败分析（矿工激励决定协议变更/区块空间定价=交易排序经济学）：https://x.com/lanhubiji/status/2086610590802510114
- **tvscreener**（TradingView Screener Python 库：6 类市场/13K 字段/Pandas/MCP 接入，1.1K stars Apache-2.0 真实项目）：https://github.com/deepentropy/tvscreener（X 推广：NFTCPS/QT9277/_zheergen 三帖）
- 共学平台 Agent API - llms.txt: https://intensivecolearn.ing/llms.txt
- 共学平台 Agent API - OpenAPI 3.1: https://intensivecolearn.ing/api/v1/openapi.json
- 链上套利残酷共学 · 21 天学习路线图（可交互站点，Bruce 发起，五阶段21课）：https://ac6dbafea1804a5692573eb28f6826f5.gz3.agentos-app.net/
- 优秀笔记分析汇总（群友整理，180条打卡精选30+篇）：本仓库 `notes/excellent-notes-analysis-20260806.md`
- 链上套利知识总结（群友整理，前两日596条打卡系统提炼，5章）：本仓库 `notes/knowledge-summary-days1-2-20260806.md`
- Agent API v1.3.0：新增 `GET /programs/{programId}/check-ins` 与按参与者查询接口，课程打卡 607 条/31 页（2026-08-06 验证）
- 精选打卡笔记汇总（54篇全文 PDF，635篇打卡精选，含索引）：本仓库 `notes/selected-notes-54-20260806.pdf` / `notes/selected-notes-54-index.md`
- 全量有意义笔记（482篇全文 PDF + 目录索引，9大主题，intensivecolearn.ing 公开打卡全量精选，生成 2026-08-08）：本仓库 `notes/full-meaningful-notes-482-20260808.pdf` / `notes/full-meaningful-notes-482-index.md` / `notes/full-meaningful-notes-482-fulltext-20260808.md`
- 增量笔记（137篇全文 PDF + 提取文本，2026-08-08 23:00 后新增打卡，9 大主题：LI.FI跨链 39 / 套利框架 31 / AMM机制 26 / 实盘数据 13 / MEV与抢跑 10 / 学习计划 5 / 做市与LP 5 / 风险与安全 5 / 其他 3，生成 2026-08-09 23:01 BJT）：本仓库 `sources/残酷共学_增量笔记_带索引_20260809.pdf` / `sources/残酷共学_增量笔记_提取文本_20260809.txt`
- 趋势追踪 MA 均线交叉策略研究复盘（v1.0.1，7页，基于 2026-08-06~07 8小时 dryRun 复盘：噪声剃头/不能无限套娃/甜点区间=15min~1h K线+$5000+/高波动趋势市/风控把PnL曲线当资产/量价验证=放量窗口才可靠；系统化交易方向，非链上套利，Paper Trading 须与实盘区分）：本仓库 `notes/ma-crossover-strategy-20260807.pdf` / `notes/ma-crossover-strategy-20260807-fulltext.md`
- LI.FI 跨链成本观测（群友教学 PDF：142次成本观测+72h价差→evidence.csv 负结论方法论）：本仓库 `notes/lifi-cost-observation-methodology.md`
- Cryptoxiao 事件交易方法论（newsliquid OI 异动策略模板：OI异动/主力占比>50%/主力盈利判定启动，$TUT 单机盘案例）：https://x.com/cryptoxiao/status/2086348730618528116 → 本仓库 `notes/newsliquid-oi-event-strategy-20260809.md`
- 缠论 K 线包含关系处理入门（技术分析教学：向上高高/向下低低原则，顶底分型前置课）：https://x.com/OtherSideBJ/status/2086296538754818425 → 本仓库 `notes/chan-kline-inclusion-20260809.md`
- 交易心理「反人性」5 次升级（另一面：猜涨跌→玩概率/一夜暴富→慢慢复利/研究市场→研究自己/情绪交易→机械执行/凭感觉→相信系统；与 gross-vs-net 方法论同主题：稳定盈利=执行系统而非预测）：https://x.com/OtherSideBJ/status/2087132116949831967
- 火山哥从0量化回测教程（X 内嵌 17 分钟视频，小白向：装工具→配环境→导策略→跑回测→看收益；免费不卖课，营销话术「活菩萨」；无方法论增量，小白向参考；⚠️ 教程类回测易忽略防假回测三坑=未来函数/屏幕价当成交价/忽略信号延迟，见 notes/day7-incremental-digest-20260811.md 笔记134）：https://x.com/huoshan007/status/2087111278984618441
- 多因子选股模型教程（公众号「数据科学实战」：价值/质量/动量三因子 z-score 标准化→加权合成→排序范式；文末知识星球引流=付费转化；方法论=传统多因子选股，与链上套利无关；z-score 量纲统一思路可借鉴于因子监控）；https://mp.weixin.qq.com/s/Ttr5fB4dqetr2-NO7qIpfQ
- Ye Su 观点：crypto 两个原生发明=稳定币 + RWA perp（「rwa 的终极形态就是 perp，大家要的是敞口是波动，现货 DTCC 自己会做」；agent 部分截断未取到全文）：https://x.com/allen_su1024/status/2087240766041264182 → 与 research-backlog 链上 perp★★ 候选直接相关
- **Polymarket 做市商无损套利漏洞**（Verilla.eth：近期新爆出、已知损失 ~$3.1 万、之前出过类似问题这次更狠、团队尚无处理方案；=「与市场无关」的机制类 alpha 实例，但规则风险=漏洞会被修）：https://x.com/CasaVerilla/status/2087051601496408322
- **上海天气「最无聊策略」实证**（0xInoo：市场几成定局时 70-80¢ 买稳赢结果→每笔 20-30%→重复 600 次→共 $4,926；无杠杆；= Paxon 天气策略同构的公开案例；容量现实：600 笔×~$8 平均 = 单笔资金 ~$30-40，绝对收益天花板低；只展示赢面=幸存者视角）：https://x.com/Inoosota/status/2087011806057259456
- **Solana TSW 机房故障事件**（2026-08-12 03:51-04:25 UTC：skip rate 飙升→29% stake 离线（>33%=宕机红线）→Jito BE 停→04:22 恢复 91% stake；TSW 年初曾 >33% 网络 stake，网络团队压到 33% 以下是 p0；Helius 验证者节点在 TSW 机房→用户发交易失败；教训：**RPC 供应商/机房级单点故障**，须多 RPC fallback + provider 健康哨兵）：https://www.teraswitchstatus.com/incidents/rb6k5l3qxw9k
- **w3player/arbitrage-playbook · LayerZero 多链套利**（NestJS+React 开源 OFT 扫描系统：LayerZero metadata 发现 OFT 资产→layerzeroscan 消息监控（3 天窗口）→OFT 合约探测（ERC-165/decimals）→sqlite 资产库→Web UI（assets/price-scan/topology/execution）；3 链 ethereum/bsc/base 公共 RPC；**已本地跑通**（pnpm install+build+node dist/main.js，POST /scan 实测发现 7 资产/13 部署，~/Code/layerzero-arb-playbook，端口 1234）；README 是 NestJS 默认模板无文档；execution 页需自备私钥未验证）：https://github.com/w3player/arbitrage-playbook/tree/main/layerzero-multichain-arbitrage
- danny 文章推荐（Dave.0xU：「像是城墙上的一个狗洞，墙外=民间交易员，钻过去=狗庄」——做市商/狗庄视角类文章，二刷三刷推荐；具体篇目未解析出，待补链接；共鸣点=理解对手方（MM/CEX 内幕机制）正是我们的研究主题）：https://x.com/bc1qDave/status/2087364076146139213
- **老白（@Wuhuoqiu）168X 长访谈**（2026-08-11，22K 字）：稳定币+Perp=两大原生发明；Perp 最后剩 4-5 家（T0=Hyperliquid，T1=Lighter/edgeX/Variational，RFQ+场外对冲机制看好）；HIP-3 是纯做市商飞轮（TradeXYZ 龙头、Paragon 后起）；「中小股票 Binance Alpha 打法」=现货控筹 60-70%+叙事+Perp 高 OI+股东 Perp 退出（长期监管套利）；Ondo Perp 在 NVDA/MU 等 RWA 大票价差滑点好过 HL/币安（场内现货对冲零延迟）；币安+火币 Perp 收入 60% 已来自美股 RWA；预测市场天花板远低于 Perp（两年走完 Gartner 曲线）；发币=负债；VC 消失论（Haseeb）；Robinhood=交易所终极形态；盘前 Perp（SpaceX/宇树）不参与（情绪博弈一根针双爆）；交易框架三次升级（系统化/尊重不知道/表达方式变多）；交易所对手方变 Robinhood/IBKR/老虎而非币安 OKX：https://x.com/168X_Fortune/status/2087072298503483477
- **fuzzland/sui-mev**（Sui MEV Bot，Rust）：`cargo run --bin arb start-bot -- --private-key {}`；支持 10 个 Sui DEX（BlueMove/FlowX/Aftermath/Cetus/Kriya/Abex/Navi/Turbos/Deepbook/Shio）+ validator relay（验证者推 mempool 交易给 bot）——与 Solana Jito、EVM Flashbots 并列的「第三种排序层形态」；定位=架构拆解研究样本（同 rust-mev-bot，**不导入私钥实盘运行**，AGENTS.md 规矩）；Sui relay ≈ Jito 的排序权拍卖变体，「新链排序竞争还没卷到」的差异化观察对象：https://github.com/fuzzland/sui-mev
- **断浪 WaveKing：Robinhood Chain 买币新手教程**（狙击玩法：CA 一发出自动买 / 提前填 CA 等池子创建冲入；链上买新币两种方式）——小白向参考、无方法论增量（同火山哥教程类）；**生态信号**：RH Chain 在吸引 meme 散户 = 链上套利候选增多（对照 0x哆啦A梦 41,945 闭环审计）；狙击 = 抢「流动性真空窗口」（DOS 加错池子同构）但纯博弈无对冲：https://x.com/waveking1314/status/2087518727436980453
- **狗哥：Uniswap v4 Hook Summer 长文**（2026-08-12）：Hook=池生命周期节点可编程（初始化/加减流动性/swap 前后），1000+ hooks initialized；六个机制方向=动态费率/JIT流动性/收益再利用/激励积分/稳定资产锚定/Launch资产；三阶段窗口=叙事发现→机制验证→流动性定价（现在=机制发现期）；四条红线=伪创新/复杂黑箱/强中心化权限/代币空转；五个信号=Hook 池真实量/路由聚合器采用/LP收益/去中心化安全/社区传播；重点项目 01(zero1cash)，观察样本 P./Sw./U./F./uT./S.——**对研究线的意义**：①JIT 流动性 Hook=订单流抢跑新形态（888BMM 线延伸）②动态费率 Hook=费率套利新候选 ③v4 新池上线=流动性真空窗口（事件驱动线）；投资框架为主，套利结构待拆：https://x.com/jingouwang888/status/2087495902005252499
- **Crypto老鹰：PM 套利机器人「泄露」晒单引流**（「GitHub 免费仓库/15 分钟 BTC 市场单日 5 万刀/用户 502 次预测胜率 98% 总利润 $54,840」）——**收益 claim 核验案例**：五问全挂（无时间窗/无本金口径/无 net 证据/单一幸存者样本/引流动机存疑，t.co 短链未展开）；对照我们 PM 线实证（0 信号日常/论文 99% 未执行/返佣三分账/BSM 对抗不了盘口效率）——「公开代码难盈利」+「晒单要看幸存者偏差」双教训：https://x.com/laoyingkhq/status/2087459121557643508
- **Suu：信息差源推荐**（「等推特刷到=别人吃剩下的」：The Block（机构数据/融资/市场结构）、BlockBeats（中文快讯）、PANews（政策/数据/研报）、Foresight News（新叙事/RWA 快）——同一条消息早看到=信息差，晚看到=新闻）；附推：7 月 CEX 总成交量 3.76 万亿环比 -23.9%、现货 -31.2% 三年低位——**工程含义**：listing_monitor 目前只抓交易所公告（一手），聚合快讯源（The Block/PANews/Foresight）未接入（二手但更快覆盖跨所异动）；信息差 = 速度分层，我们的哨兵已经在「公告层」，可评估加「快讯层」：https://x.com/Suu766/status/2087508712873513327
- **Lonely：Alloomi 自进化数字员工三层拆解**（「检索≠学习」：RAG 在 10M token 规模准确率跌至 24.9%；Holistic Context=5-tuple 归因+承诺生命周期状态机+赫布联想；SEA=Engram X 稀疏残差注入+三阶段后训练飞轮；九项 benchmark 记得住/学得会/做得成）——AI Agent 架构线（qintopia-agent-os 参考）：把「记忆系统」从检索升级为「经验内化」，MAPE 回滚=工程质量闸门；与 Hermes 的 memory/skills 机制同构（但 Hermes 是外部记忆，Alloomi 是注入模型权重）——「一个 Agent 能找到过去，不代表它能从过去中学习」：https://x.com/Lonely__MH/status/2087715951219487033
- **链上小帅：三款免费开源 PM bot**（「付费版 5-6 千美元 → GitHub 全开源，Claude Code 改配置模拟跑通才实盘，功能正常」——GitHub 链接在视频/评论区未展开；t.co 指向推文本身）——**已识别生态**：OctoBot-Prediction-Market（Drakkar-Software 成熟团队，105★，GPL-3.0，含双边<1 套利/复制交易/paper trading）、MrFadiAi/Polymarket-bot v3.1（4 策略：arb/dip arb/smart money，TS）、kenfri13 15-min Up/Down arb（Gamma API）、Benjam1nCup python-V2（7 策略+双边套利文档）、PolyCatalog 收录 30 个——**对照我们的扫描器**：别人的「buy both sides < $1」= 我们的盘口可执行组合检测；开源≠盈利（048 结论+我们 0 信号日常）；这条没晒收益，可信度高于 Crypto老鹰那条；待办：拉 OctoBot 源码对照盘口/结算处理：https://x.com/LSXS_888/status/2087747990958018856
- **µToken（@uTokenPro）**：新代币标准叙事（ERC20 带图像/NFT 化，「Coming soon to Robinhood + Uniswap」）——上市前事件，**新币套利提前准备清单**：①listing_monitor 需加 DEX 新池子源（µToken 上 Uniswap 是 DEX 上市，现监控只覆盖 CEX 公告）②DOS 四开关预检（新标准初期能买/能提/能跨/能卖分钟级变化，状态带时间戳）③假币清单先行（新标准=假币重灾区，DAPPOS 实锤）④流动性真空窗口（DOS 加错池子同构，挂 alpha 限价单没接到不亏钱）：https://x.com/uTokenPro/status/2087628867284664657
- awesome-systematic-trading（wangzhe3224 整理，4.8K star，系统化交易工具分类：回测/实盘框架 backtrader·Freqtrade·vn.py·Nautilus、AI量化 FinRL·Qlib·FinGPT、因子/数据/风控/接口/书籍，跨股票·加密·期货·期权·外汇）：https://github.com/wangzhe3224/awesome-systematic-trading → 本仓库 `notes/awesome-systematic-trading-20260809.md`
- Robinhood 套利全景审计（0x 哆啦A梦：3天 259 万区块/1327 万 Swap 扫出 122 候选系统，41,945 闭环毛价差 10 万 U；四套系统——首笔价差(98.7% 同区块内)/条件执行/确认后尾气(残差≈0)/覆盖空白；「机器人收入写在覆盖的池子和路线里」；真实利润=毛价差-成功Gas-失败空跑Gas-滑点-库存损耗-延迟-调仓）：https://x.com/hunterweb303/status/2086339604081508638 → 本仓库 `notes/robinhood-arbitrage-audit-20260809.md`
- Bruce Hermes 技巧：回复尾部显示 model/reasoning_effort/session 状态（全局规则，Telegram runtime_footer 配置，官方文档已支持 `display.platforms.telegram.runtime_footer`）：https://x.com/brucexu_eth/status/2086271718306496594 → 本仓库 `notes/hermes-runtime-footer-20260809.md`
- Polymarket 量化机器人解剖（0xPINK3：香港量化工程师用 Claude 搭的机器人，102 天/29,115 次预测/60% 胜率/日均 $2,370/共 $241,767；短线加密涨跌市场循环，每小时 ~12 笔；=时间套利+对冲方向性敞口+库存轮换组合；先建一侧仓位等概率变动再补另一侧；「许多小幅优势随时间积累」）：https://x.com/0xPINK3/status/2086337881451475451 → 本仓库 `notes/polymarket-claude-bot-241k-20260809.md`
- Solana 私钥安全（ACCU.CC 长文：Trust Wallet 供应链攻击 700 万 U / 剪贴板攻击 / Coding Agents 新威胁 / 私钥生成随机性 / 保管方案对比——纸钱包·金属钱包·链下多签 naive(拆段不安全)·elder(加法分片)·Shamir SSS / 量子攻击——Solana 地址=公钥无法抗量子，须密切关注）：http://accu.cc/content/solana/prikey_security/ → 本仓库 `notes/solana-prikey-security-20260809.md`
- Neutral Trade 自动驾驶仪（Solar 社区推广：自动学习策略组合、跟踪表现、判断回撤、自动切换最佳策略；一次存款覆盖多元策略中性收益；9/1 前免手续费+NT 积分）：https://x.com/solana_zh/status/2085908862972887258 → 本仓库 `notes/neutral-trade-autopilot-20260809.md`

## Hermes

- Hermes Agent 中文入门教程: https://tutorial.hermescn.org/
- Hermes 安装与首次运行教程（Bruce 文中精确链接）: https://hermescn.org/docs/tutorials/guide/install-and-first-run
- Hermes 官方文档: https://hermes-agent.nousresearch.com/docs
- Hermes GitHub: https://github.com/NousResearch/hermes-agent

## 套利基础

- 套利 - Wikipedia: https://zh.wikipedia.org/wiki/%E5%A5%97%E5%88%A9
- 小隐寺投资百科（美股/期权/加密 915 词条）: https://xiaoyinsi.com/wiki
- 链上实验课 - 可执行 Uniswap V2 双语课程（18 课）: https://onchain-lab-lessons.vercel.app/
- 携手 AI 学套利（AI 辅助套利框架学习书：价差≠利润 / 四道检查 / 套利vs相对价值vs投机vs做市辨析 / 机会为何仍存在；首章「价差为什么不等于利润」约18分钟，与本项目"报价≠成交"主线同构；框架已 distill 为 notes/arbitrage-framework-first-principles.md）: https://learnwithai.robotisland.cloud/?lang=zh-CN
- taoli.tools: https://taoli.tools/
- 1Token - 资金费套利策略: https://zh.blog.1token.tech/crypto-fund-101-funding-fee-arbitrage-strategy/
- w3player/arbitrage-playbook issue #1 - 抛砖引玉（待整理）: https://github.com/w3player/arbitrage-playbook/issues/1
- w3player/arbitrage-playbook issue #2 - 资金费套利简单介绍（待整理）: https://github.com/w3player/arbitrage-playbook/issues/2
- w3player/arbitrage-playbook · LI.FI 六种套利模型（README+00-06 全套：预置库存/稳定币价差/同链DEX/Solver价差/跨链循环/Earn轮动，含统一风险表与回测要求，2026-08-07 归档）: https://github.com/w3player/arbitrage-playbook/tree/main/lifi/docs → 本仓库 `notes/lifi-playbook-w3player-20260807/`
- hunterweb303 - 收过路费策略原帖: https://x.com/hunterweb303/status/2084815706252841060
- BlockTempo - 三明治之王 JaredFromSubway 被 66 个假合约蜜罐反杀（750-1500 万美元，Karma 案例，2026-06-21）: https://www.blocktempo.com/jaredfromsubway-mev-bot-honeypot-drain-7-million-fake-token-contracts/ → 本仓库 `notes/jaredfromsubway-honeypot-20260807.md`
- sunlc.ai - 老孙金融终端系统4：套利监控模块设计思路（正/负费率、美股、RWA、升水、OI异动、自定义；"全自动100%踩坑"、OI不可造假）: https://x.com/sunlc_crypto/status/2085578921320616081 → 本仓库 `notes/arbitrage-monitoring-terminal-sunlc-20260807.md`

## 监控和数据 API

- ccxt: https://github.com/ccxt/ccxt
- ccxt 文档: https://docs.ccxt.com/
- DexScreener API: https://docs.dexscreener.com/api/reference
- Birdeye API: https://docs.birdeye.so/
- python-telegram-bot: https://github.com/python-telegram-bot/python-telegram-bot

## 数据和路由

- LI.FI: https://li.fi/
- LI.FI Docs: https://docs.li.fi/
- Bruce - LI.FI 跨链套利执行与 Agent 集成长文（本人 X 发布）: https://x.com/brucexu_eth/status/2085022891339554855
- Jumper（LI.FI 的 to-C 聚合交易产品，同链/跨链报价和路径）: https://jumper.xyz/
- LI.FI Builders Telegram: https://t.me/lifibuilders
- The Graph: https://thegraph.com/
- ETHGlobal ETHOnline 2026 prizes: https://ethglobal.com/events/ethonline2026/prizes
- 聚合器对照（报价/路由竞品研究）: KyberSwap https://kyberswap.com/ ｜ 0x https://0x.org/

## MEV 和安全

- DeFi PoC Lab - MEV攻击: https://starkxuns-organization.gitbook.io/defi-poc-lab/amm-specific-attacks/mev-gong-ji
- DeFi PoC Lab - MEV攻击 Markdown: https://starkxuns-organization.gitbook.io/defi-poc-lab/amm-specific-attacks/mev-gong-ji.md

## Solana

- Solana Docs: https://solana.com/docs
- Solana Foundation Developer Courses: https://github.com/solana-foundation/developer-content/tree/main/content/courses
- Solana Explorer: https://explorer.solana.com/
- Solana Playground: https://beta.solpg.io/
- Solana Cookbook 中文版: https://solanacookbook.com/zh/
- creatorsand/solana-co-learn: https://github.com/creatorsand/solana-co-learn
- Anchor Docs: https://www.anchor-lang.com/docs
- Anchor Installation: https://www.anchor-lang.com/docs/installation
- Anza Wallet Adapter GitHub: https://github.com/anza-xyz/wallet-adapter
- Anza Wallet Adapter Example: https://anza-xyz.github.io/wallet-adapter/example/
- Solana issue #27598 - cargo-build-bpf/sbf: https://github.com/solana-labs/solana/issues/27598
- Solana issue #34987 - solana-program rustc mismatch: https://github.com/solana-labs/solana/issues/34987
- Solana Transaction Fees: https://solana.com/docs/core/fees
- Solana Compute Optimization: https://solana.com/developers/guides/advanced/how-to-optimize-compute
- Solana Geyser Plugins: https://docs.solanalabs.com/validator/geyser
- Jito Docs: https://docs.jito.wtf/
- Jupiter Developer Docs: https://dev.jup.ag/
- Titan DART Swap API Access: https://titan-exchange.gitbook.io/titan/developer-doc/dart-swap-api/get-api-access
- Raydium Docs: https://docs.raydium.io/
- Orca Docs: https://docs.orca.so/
- Meteora Docs: https://docs.meteora.ag/
- Drift Docs: https://docs.drift.trade/
- Lighter（Solana 订单簿 DEX，ZK prover 架构，链下撮合链上结算，高 TPS 执行层，套利执行场所候选）: https://lighter.xyz/
- Lighter Prover Challenge（社区活动，跑 prover 节点提升网络吞吐；@nimberland001 成首个 TPS>30000 参与者）: https://lighter.fast/
- Pyth Docs: https://docs.pyth.network/
- Helius Docs: https://docs.helius.dev/
- Helius Dashboard: https://dashboard.helius.dev/login?redirectTo=/dashboard
- Triton One Docs: https://docs.triton.one/
- Solscan: https://solscan.io/
- WebSocketKing: https://websocketking.com/
- Solana Notion 资料页: https://attractive-spade-1e3.notion.site/Solana-fca856aad4e5441f80f28cc4e015ca98
- rust-mev-bot Quickstart: https://rust-mev-bot.solboxs.com/getting-started/quickstart
- rust-mev-bot shared GitHub: https://github.com/SaoXuan/rust-mev-bot-shared

## 信息差

- four.meme（BNB 链 meme 发行平台，pump.fun 类）: https://four.meme/zh-TW
- flap.sh（BSC 可编程 Token Launchpad）: https://flap.sh/board
- pump.fun（Solana meme 发行，explore=新币榜；API frontend-api.pump.fun 被 Cloudflare 1016 挡）: https://pump.fun/explore
- gmgn.ai（meme 交易监控/聪明钱，Solana 等链，Cloudflare 保护需登录）: https://gmgn.ai/?chain=sol
- debot.ai（meme 信号/自动交易平台，多链）: https://debot.ai/
- awesome-remote-job: https://github.com/lukasz-madon/awesome-remote-job
- ai-money-maker-handbook: https://github.com/XiaomingX/ai-money-maker-handbook
- awesome-systematic-trading: https://github.com/paperswithbacktest/awesome-systematic-trading
- ai-goofish-monitor: https://github.com/Usagi-org/ai-goofish-monitor
- TelegramGroup: https://github.com/AZeC4/TelegramGroup
- 量化供应商选型指南（PPT 营销套路拆解，原野之诗 2026-08-08）: https://mp.weixin.qq.com/s/M3WlFiiHDC6l5So-Fi-8Ww

## 自媒体/公众号

- 想赚钱的坤 - 公众号工具站合集（①零克查词 lingkechaci.com 违规词/敏感词 ②易撰 yizhuan5.com/app/trends 原创查询+热点监控 ③文章下载 down.mptext.top（有 API）④创作罐头 czgts.cn/daily_hots/list 低粉爆款+素材选题）：https://x.com/kun66666677/status/2086722055601606928 （2026-08-10 验证四站均在线 HTTP 200；**易撰 CLI 查无实据**——官网无 docs/API/openapi 页，官方只提「数据定制+API 接口服务」需商务洽谈，推文 CLI 说法是推广话术；零克查词站点主推小红书/抖音/B站，公众号违规词覆盖不全；down.mptext.top=微信后端 API 代理站，searchbiz/appmsgpublish 接口需扫码登录，已据此写了直连免登录抓取脚本 `scripts/fetch_wechat_material.py`）
- 辣条味奶糖. - 三款「去 AI 味」写作 Skill 推荐（桥接文档组合法：三个 skill 各出一版→人工对比融合）：https://x.com/spicycandy00/status/2086732942806847701 （2026-08-10 核验：①KKKKhazix/human-writing 2.2K★ 中文去AI味通用改稿 ✓ ②op7418/Humanizer-zh **15K★**（推文链接贴错成 guizang-ppt-skill PPT 仓库，正确仓库是 Humanizer-zh，英文 Humanizer 汉化版 Claude Code Skill）✓ ③lijigang/ljg-skills 6.8K★ 概念解释型，非专做去AI味 ✓；用户本机已有 `humanizer` skill 同方向；已建桥接文档 `templates/writing-skill-bridge.md`（不安装直接调用+选择逻辑））

## 套利实盘/案例

- 在金 - vibe-coding 网格系统实盘记录（4 交易所 6 账户，1 个月 1300U；更新：只看实盘交易所/主页仓位+强平价/Decibel APT 手续费报错+API 钱包地址+复制；承诺 Arcus 规模入场后开源）：https://x.com/zaijin338191/status/2086779614068896068 （2026-08-11 回测验证幸存者偏差：`notes/grid-trend-backtest-20260811.md`——13 个月下跌月亏损率 75-88%、非下跌月盈利率 100%；0 手续费全期仍净亏，趋势月库存浮亏 > 震荡月 gross edge；10x 杠杆 2026-01 -10% 月即爆仓。评论区 YuChen 4 个月赚几千刀/本金几百刀同属状态选择偏差样本）

- Leo｜一个人+AI - Polymarket 排行榜收益是税前口径（官方手续费=股数×0.07×p×(1-p)，仅吃单方付费；榜前 4 地址真实到手仅 24-45%，榜单排序与真实排序相反；返佣=手续费占比，返佣多=手续费多=坏信号；验证法：链上重建净持仓→官方结算核销→对照曲线日变化）：https://x.com/runes_leo/status/2086688671244472701 （2026-08-11 归档：与 L0007 成本拆解同主题——gross vs net PnL、费用与 edge 同量级可翻正为负，与网格回测「0 手续费仍亏」互证；taker 判断法=吃单为主必须减费）
- 甜甜 Sweety - tvscreener：TradingView Screener 数据源 Python 库（可视化代码生成器，13000+ 字段，股票/加密货币/外汇/债券/期货）：https://x.com/tiany221/status/2086656283420319771 （2026-08-11 已验证 ✅（`notes/tvscreener-verified-20260811.md`）：直连可用无需代理，美股实时价/涨跌幅/1h 全有，CryptoScreener 含 BITGET:RSPYUSDT 等 tokenized 股对——币股时钟差双腿数据源打通；坑：按 ticker 过滤需交易所前缀+本地过滤，isin 不支持多值）
- 另一面 - A股 7 步短线复盘框架（情绪→连板梯队→板块结构→资金流向→亏钱效应→风向标→明日计划）：https://x.com/OtherSideBJ/status/2086960739953316029 （2026-08-11 归档：结构可迁移——亏钱效应=尾部风险优先，风向标=哨兵先行，复盘→推演=假设验证循环）

## 套利实盘/案例

- 0x 哆啦A梦 - Pons 土狗开盘手法拆解（Dev 3929…ec4a 的 140 盘链上取证：建仓~3.8ETH → 藏仓59%库存转子钱包 → 试探小卖0.4-1% → 主卖40%初始库存 → 子钱包清仓 → Locker 分润；35盘中31盘有分润回执；「四本账」监控=库存/距高点距离/5-20-100区块外部净流/受控动作间隔；吃的是 Dev 还在做盘 vs 已兑现的时间差；进出规则=费后正数才进、受控大卖出现就走）：https://x.com/hunterweb303/status/2087000359395033124 （2026-08-11 归档，拆解见 notes/meme-dev-harvest-pattern-20260811.md）

- 李胜利 - Hyperliquid 资金费率套利案例（1wu 保证金 16000u 单边仓位，最高 1200% 年化；实操笔记 02「先搭监控再找策略」——最开始学套利把时间花在找策略上，实测后才明白顺序反了）：https://x.com/web3_LiShengli/status/2086756851027456359 （2026-08-11 归档：监控>策略与哨兵架构一致；但「单边仓位」=赌方向非对冲，1200% 年化需按 gross/net+窗口口径批判——与 Polymarket 榜一、网格 1300U 同款幸存者偏差风险；Hyperliquid funding 可作为 funding 哨兵新数据源候选）

- nine_ - 妖币热潮中后段操作思路（埋伏高控未异动+低市值壳币+回踩追龙头；追涨幅榜=断头台接盘；「5M 壳价难亏钱」假设：$ptb 7M/$act 10M/$griffain 10M/$light 6M，只要不偷偷增发；高控币 $evaa/$collect 彩票；评论区「你能想到的庄家也早已料到」）：https://x.com/nine_DeFi/status/2086645465114927601 （2026-08-11 归档：与 Dev 出货状态机互为玩家侧/庄家侧；「5M 难亏」是可回测假设）

- 另一面 - 缠论走势类型快速入门（走势只有两类：盘整=1 个同级别中枢 a+A+b / 趋势≥2 个同方向同级别中枢 a+A+b+B+c；方向按进入段 a 定；走势终完美=开始必结束、结束后转其他类型；「走势不可预测但可完全分类」；先给已完成走势贴标签，不预测生长中走势）：https://x.com/OtherSideBJ/status/2087003997853086136 （2026-08-11 归档：分类学哲学与「只读已发生、不预测」一致；中枢≈无套利带走廊、趋势≈出轨后持续偏离的跨域映射）

- 青蛙💎 - BTC 资金费率套利保姆级教程（488 天净赚 9.57% APR~7.16%；统一账户现货多+永续空 1x delta 中性；历史数据 2019-09→2026-08 共 7578 次结算 85.58% 正/14.42% 负，84 月仅 6 月合计负、最差 2020-03 -0.53%，2026 2-4 月连续三个月负；成本=价差+基差+四笔手续费；省费四招=返佣/VIP/BNB 抵扣/Maker；安全结构=不借币+1x+数量配平+独立子账号）：https://x.com/BlockBloomer/status/2087068515220308232 （2026-08-11 归档：research-backlog #2 同所资金费率套利的完整实操答案；「连续负费率期」是最大尾部风险；7.16% 是诚实口径，对比 1200% 年化类晒单）

- brucexu.eth - 转发点赞「Hermes 登顶背后：一个 Web3 团队的进阶之路」研究报告（@0xjacobzhao 分析：Hermes 现象级增长=挑战者增长系统——承接 OpenClaw 教育成熟的用户池+闭合「可委托性」；Bruce 贡献了个人体验反馈）：https://x.com/brucexu_eth/status/2087078955228762263 （2026-08-11 归档：AI Agent×Web3 交叉参考，与 qintopia-agent-os 框架设计潜在相关；原文链接 x.com/0xjacobzhao/status/2087070528003858829）

- Solar (Solana 中文社群) - 在 Solana 上构建 Kalshi/Polymarket 风格预测市场教程（QuickNode 原推：Anchor + Quasar 完整可运行代码、动画演示；学预测市场机制 + 商业逻辑→Solana 账户结构）：https://x.com/Solana_zh/status/2087092893320163500 （2026-08-11 归档：接 Polymarket 研究线（供给侧理解）+ D2 Anchor 环境实操；候选补课「Solana 预测市场实战」）

- 403-error - 为什么聊天系统要为 Agent 重做一遍（Raft/slock AI-Native 聊天系统分析：消息 CLI+Freshness Hold 二阶段验证（序列变了扣住存 Draft 让 Agent 二次决定）；Agent Inbox（信号/正文分离控上下文）；群聊静音；单 Agent 单 append-only 上下文（身份连续+prefix caching）；Multi-Agent 反思=超级 Agent 内部也要调度、个体绑定让「名字标记权利/身份标记责任」）：https://x.com/SiyuYeAndy/status/2086751636081545456 （2026-08-11 归档：qintopia-agent-os 多 Agent 通信层设计直接参考——消息一致性+上下文控制是核心难点）

- Paxon 提供 - Binance 官方 OPENAIUSDT Pre-IPO Perpetual（2026-05-26 上线）：Pre-IPO 永续品类，交易 OpenAI 未来公开市场估值预期；不代表真实股权、不挂钩任何现货代币（2026-08-11 归档：事件驱动价格发现品类；无现货锚=无法期现套利；与 Polymarket 二元结算结构对比）

- Paxon 截图 - 现货 OPENAI/USDT vs 合约 OPENAIUSDT Pre-IPO 永续对比（现货=独立订单簿的合成/主题代币；永续=基于 OpenAI 上市估值预期的合成衍生品，USDT 现金结算无实物交割、标记价±1%规则+资金费率、IPO 取消可能结算下架；两者无可转换/可交割/可赎回关系 → 价差巨大但**不存在无风险套利空间**）（2026-08-11 归档：假价差案例 #2——与 Binance Alpha「聚合器 vs 单池」同构：没有可转换/可赎回闭环的价差不是套利）

- 投机实验室 - TradingView 免费 DIY Custom Strategy Builder 指标（免费白用：指标搜索栏输入「DIY Custom Strategy Builder」收藏添加；自由组合领先指标组/确认指标组/开关版等多指标策略）：https://x.com/LabSpeculation/status/2087111347972747501 （2026-08-11 归档：与 tvscreener 同源（TradingView 生态）；可作策略可视化/快速验证工具，注意社区指标非官方策略引擎）

- brucexu.eth - 转发 siddontang《AI 编程缺失的抽象层》并引用自己 6/18 推文（编程发展本质=降低「意图→机器执行」的转换成本：19 世纪初打孔纸带 → 1940s 二进制机器码 → 1950s 汇编 → 1950s-2020s 高级语言/框架 → 2020s 自然语言→AI；siddontang 原文章：年轻程序员可能最危险，理由=过去工程师从修 Bug/写代码中积累的抽象能力被 AI 短路；siddontang 后续：将来不可或缺的课是写作课，能清楚表达观点的人能更好地用 AI）：https://x.com/brucexu_eth/status/2087344913398808612 （2026-08-12 归档：与用户理念「人需判断力驾驭 AI（否则=抽卡）、AI 是加速器非方向盘」直接互证；「表达清楚=更好用 AI」支持 writing-skill-bridge 方向——写作=把意图结构化的能力，是 prompt 质量的上游）
- 想赚钱的坤 - Starter Story 真实收入项目数据库（收录 2996+ 已产生真实收入的项目：AI 工具/SaaS/插件/电商/内容网站/一人公司，每条含怎么开始/收入/成本/启动时间/技术栈/增长渠道；已被 HubSpot Media 收购改版 2.0 BETA；宣称 $3B+/mo 合计收入）：https://x.com/kun66666677/status/2087737224804868253 （2026-08-13 归档：短链展开=starterstory.com；**数据口径=创始人自报（Verified — self-reported by the founder），无第三方审计**——与 5 问筛选「税前口径/样本窗口」同款风险，榜单 gross 收入不含成本；对套利研究线直接价值低，间接参考=商业模型拆解/一人公司收入结构，与 AI-first data flow 商业观可对照）
- 黄小木（转发 Roland.W） - 《我找到了AI写作问题的根源（附解法）》：AI 味两大根源=①自造说法（「执行成本塌方」「先蹲两周侦察」需读者先理解说法再理解事实，未增信息即删）②英译式话语连接（reflecting→「反映出/体现出/表明了/这意味着」，主语错位+事实/解释/结论压一句，负迁移自英文语料主导）；解法=原材料先分三类（事实/可支持解释/未答问题）→ 删连接词分别读前后句 → 用「原因是/与……同时出现/可能与……有关」精确化关系 → 不让 AI 直接润色，让它并排输出「原句/问题/建议」人再核对：https://x.com/ai_xiaomu/status/2087181801458716889 （2026-08-13 归档：与去 AI 味写作线直接相关——比 human-writing「删金句/材料第一」更细一层，补连接词级检查规则，可并入 writing-skill-bridge；评论区微尘印记：dbskill 先识别特征再带着改，AI 直接改=左手倒右手，与「不要直接润色」互证；黄小木转语「看是矛锋利还是盾强」=自家去AI味工具对比）

- Adrian Punk - Punk XHS Cards：把 Markdown 长文转成小红书竖版知识卡片（个人 IP 确认→主题色→封面→内页，三个确认门；教程关键截图按「步骤—截图—完成标志」保留；输出 1 封面+9 内页+publish-copy.md 标题正文 tags+ZIP；Python/Pillow 跨平台渲染）：https://x.com/AdrianPunk115/status/2087494970341257385 （2026-08-13 归档：GitHub 仓库 adrianpunk/punk-xhs-cards 8 commits 实测通过——smoke_render.py 真出 cover.png+card-01.png，SKILL.md+9 references+10 scripts 完整；推文「3 个月赚 8 位数」为营销口径未验证；对共学公众号文章多平台复用有参考价值，与 ObsidianMain 分发管线思路同构，可作 D 系列文章跨平台候选工具）

## 资金费率查证

- Bitget 资金费逃票查证（官方规则核验 + 实测：主流 8h 结算钳制±0.01%/往返费0.12%倒挂→100期0期净正；长尾4h结算高费率全负费率=事件非策略；文档时点07/15/23 vs 实测08/16/00差1h）：本仓库 `notes/bitget-funding-fare-dodge-verify-20260815.md`（数据源 Bitget API v2/mix/market）

## 共学增量笔记批次

- 资金费率监控平台×5（Loris/CoinGlass/Sharpe/PerpFinder/CoinBeacon 全存活实测 200；口径差异教学：PerpFinder 1h vs CoinBeacon 8h vs Sharpe APR 归一化=口径不统一对比即错误）+ Auto-Prediction AI 语义套利 Harness（TraderAlice 仓库真实 8★ pre-alpha、禁实盘、bigint 定点+fail closed 安全设计、settlement contract≠world probability 本体论=PM 结算风险理论化）：https://x.com/WY_mask/status/2089474853317603339 + https://x.com/0xcherry/status/2089351523117715906 → `notes/funding-platforms-and-auto-prediction-20260818.md`
- 2026-08-17 增量 91 篇 digest（RWA 基差套利框架 log(P/X)分解+周末价格发现权、资金费短窗口高估9.3倍+换手率判据、Monad gasLimit不退回、测量自我污染滑点指纹、原子性≠执行权竞价让渡95%、OEV/SVR双聚合器机制、充提状态套利、机会评分可审计化、ADR扩散vs收敛=强制收敛机制、三角套利延迟三层优化、Quoter幻觉率99.5%）：`notes/icl-incremental-notes-digest-20260817.md` · `sources/残酷共学_增量笔记_带索引_20260817.pdf`
- 2026-08-15 增量 108 篇 digest（SVR/Atlas 89% 清算奖励回收、LVR σ²/8、Qy 庄家行为三层研判、RPC 新鲜度 vs 延迟、PM 争议期）：`notes/icl-incremental-notes-digest-20260815.md`
- 2026-08-16 增量 90 篇 digest（状态跳变第10类机会、v3 腿 Quoter 纪律、LI.FI 路由每周翻转+USDC.e deny list、funding 躺平证伪+OI 过滤、HL 币股结构性基差、天然价差基线、ADL 剪刀仓）：`notes/icl-incremental-notes-digest-20260816.md`
- OKX 闪赚+现货对冲评估（白云 issue #6；Paxon 实测 RE 单币：宣传年化 120%→实际 34%，gross vs net 实证；事件驱动家族，不进主攻）：`notes/okx-flash-earn-hedge-arb-20260817.md`
- Robinhood Chain FCFS 排序（官方文档：先到先得无 priority auction→拼延迟不拼 gas；互证 0xd7121208 纯延迟竞赛；未来变量=Timeboost 0.5s 竞价）：`notes/robinhood-chain-fcfs-ordering-20260817.md`
- 套利软件安全实操手册（@web3_LiShengli 笔记05：一键套利软件=私钥即钱包控制权；开源≠安全；GPT 七问拆解代码提示词；签名类攻击 Permit/setApprovalForAll 无 gas 授权；剪贴板地址替换；独立助记词才算隔离）：https://x.com/web3_LiShengli/status/2089256791737798914 → `notes/arb-software-safety-playbook-20260817.md`
- Lighter × Robinhood Chain 价差套利（@Chosmos110 蚂蚁搬家 200U→2U 实盘；事实验证：Robinhood Chain 7/1 主网+Lighter 为钱包默认 perp DEX+1100 万 LIT 激励+积分 2x = 新订单簿补贴期候选；工具未开源先当推广；Points 条款禁机器人）：https://x.com/Chosmos110/status/2088944635255406716 → `notes/lighter-robinhood-chain-arb-20260817.md`

- [W3「无风险套利教程 @1inch」2026-08-16](https://x.com/w3_888/status/2088821704483377268) — 8.9万views，ETH 1000 USDC→1003.42 USDT(+0.34%)截图；核验：常态价差仅0.045%(CoinGecko 08-16)，0.34%是脱锚事件窗口非常态；扣gas后常态净亏，「每天10U」需$3k+本金+等事件；标题「无风险」有水分（见 notes/oneinch-usdc-usdt-claim-20260816.md）
- [1inch Aqua 激励套利拆解 @NeoWeb3Nova 2026-08-19](https://x.com/NeoWeb3Nova/status/2089920914661982670) — X Article 全文：1inch Aqua Maker/LP Incentive Program 机制（资产留钱包 allowance 多 Position、奖励按 processed volume 分配非 parked TVL、1000 万 1INCH+50 万 USDC DAO 池、wallet cap/wash-trading filter/自交易可暂停市场）；@w3_888 两周 3 万 U 自述 + 8/2 价格核对「血泪教训」；分类=Incentive Arbitrage（非撸毛非价差套利）；「无风险」被作者自己纠正（官方明示 IL/Inventory/合约风险）；与 08-16 旧归档（w3_888 USDC→USDT 0.34% 有水分）同一人，gross vs net 标注样本不足；群内已有人参与 aqua（日书0817[39]）：`notes/oneinch-aqua-incentive-arb-20260819.md` · `sources/oneinch-aqua-incentive-arb-article-20260819.txt`
- [宇树、长鑫上线币安合约：交易所和券商必有一战 C Labs 2026-08-19](https://mp.weixin.qq.com/s/2jo_Lp5ct2oGkHEC4nb3Rw) — 现金结算合成衍生品≠股票代币（无股权/分红/投票权，「股票猜涨跌」）；时间线：7/14 Trade.xyz HIP-3 Pre-IPO CXMT 5美元→7/16 长鑫 IPO 发行价 8.66 元→7/31 MEXC 先上 UNITREE 10x→8/4 HL 宇树开盘 73.33→币安最后上；绕监管=不发行证券；8h 资金费示例 -2% 万U多头白拿200U；长鑫炒到发行价近 6 倍/宇树 4 倍+（海外无法跨市场套利拉平）；纳斯达克 23h 制 2026-12-06 上线；互证 0817 digest 083 RWA 基差（Tokenized Stock×HIP-3）+ 日书[48]打新 + [99] Bybit 平台随意改指数/周期：`notes/cex-vs-brokers-stock-perps-20260819.md` · `sources/宇树长鑫币安合约_CEXvs券商_Clabs_20260819.txt`
- 2026-08-17 打卡日书全量 161 章（52 位伙伴，22.6 万字符；HTML 书导出：每篇含作者/字数/分类/主题；91 篇增量笔记 PDF 批次之外独有 93 章，含 x-tan-dev Mint 旧 Reserve 14K、QQQ Lighter 双市场 8.2K、starkxun 资金费对冲 11K、cfanboy Inventory 11.6K、Web3Rason 全自动系统 14.5K、SimonQuant Tokenized Stock 15K、HerschelLi-31 MEV 全景 20K 等）：`sources/残酷共学_打卡日书_20260817.html` · `sources/残酷共学_打卡日书_提取文本_20260817.txt` → `notes/icl-daybook-0817-supplement-digest-20260819.md`
- [Ponytail 官网 2026-08-19](https://ponytail.dev/) + [GitHub DietrichGebert/ponytail](https://github.com/DietrichGebert/ponytail) — AI 编码 agent「懒人规则集」（YAGNI 阶梯），**105,599 stars（2026-06-12 创建，GitHub API 实测）/ MIT / v4.9.0 / 20+ 宿主适配**；拆解价值=规则分发架构（skills/ 单一源 + AGENTS.md compact 版 + plugin/skill/instruction 三级 + 指令构建器 + 适配器测试矩阵 + platform-native 知识配套）；Hermes 适配=plugin.yaml + __init__.py（pre_llm_call 注入 / pre_gateway_dispatch 命令重写 / register_skill 命名空间），218 行全拆解；对 qintopia-agent-os 借鉴清单 8 条；泼冷水=benchmark 自报（12 任务中位数）、规则注入是提示词非约束、适配器矩阵=持续税、instruction-tier 静态文本名不副实：`notes/ponytail-rule-distribution-20260819.md`
- 2026-08-25 brucexu_eth 2091888306049220613: BNB Chain v4 Hook 恶意池技术分析(模拟0%真实12.8% LP fee, gas环境攻击三机制, 聚合器从业者视角) → 08-24 事件机制深挖, 报价核验加 gas 指纹
- 2026-08-25 laoyingkhq 2092145302568571195: 天气套利返佣帖(via=YINGGE888+PolyCop跟单) 数字不采信; 机制属实=PM天气市场锚定NOAA机场站vs市中心温差
- 2026-08-25 brucexu_eth 2092151224359346259: 套利周会纪要(双边对冲/TUT爆仓捡尸体/1inch/Agent自动化), 可能最后一期
- 2026-08-25 solana 2091918581755793520: Virtuals agent tokenization 上 Solana(官方)
- 2026-08-25 Chosmos110 2091868979505832157: Lighter价差套利工具 200U→2U实盘(小样本)
- 2026-08-25 WY_mask 2092013078263079227: perpdexlist.com/arbitrage 年化计算器 + 5费率平台合集(JS渲染待核验)
